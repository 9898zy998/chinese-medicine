...
CRF_MODEL_PATH = 'checkpoints/01/47.pt'
BERT_PATH = r'bert_wwm_5'

class CRF(object):
    def __init__(self, crf_model, bert_model, device='cuda' if torch.cuda.is_available() else 'cpu'):
...
crf = CRF(CRF_MODEL_PATH, BERT_PATH)
...
if __name__ == "__main__":
    text = '伏翼实禽也，由虫鱼部而移焉。橘柚附于果实，食盐附于光盐。生姜、干姜，同归一说。至于鸡肠、繁缕、陆英、蒴 ，以类相似，从而附之。仍采陈藏器拾遗、李含光音义，或讨源于别本，或传效于医家，参而较之，辨其臧否。'
    print(text)
