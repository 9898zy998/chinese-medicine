...
if __name__=="__main__":
    ...
    parser.add_argument("--trainset", type=str, default="data/神农百草经_labeled.txt")
    parser.add_argument("--validset", type=str, default="data/神农百草经_labeled.txt")
    ...
    dataset = NerDataset(hp.trainset)
    test_percent = 0.3
    length = len(dataset)
    train_dataset, eval_dataset = torch.utils.data.random_split(dataset, [int(length * (1 - test_percent)),
                                                                          length - int(length * (1 - test_percent))],
